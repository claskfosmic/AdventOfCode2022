...
#        current directory, then makes that directory the current
#        directory.
...
class puzzle:
	...
	def handleInput(self):
		...
		# Start without a current path
		#
		currentPath = ""

		# Create an array holding the 'path' we're in
		#
		self.path = ['/']

		# Create an object to hold the full filesystem
		#
		self.filesystem = {}

		for line in self.lines:

			currentPath = self.path[-1]

			# Check if the line starts with '$ cd '
			#
			if line[:5] == '$ cd ':

				# If so, when the line continues with '..', we must move up a place
				# in our path, so pop the last element, meaning we're currently 1
				# directory higher in the hierarchy.
				#
				if line[5:7] == "..":
					self.path.pop(-1)
					currentPath = self.path[-1]
					#print ("Back to path %s" % self.path[-1])
				
				# Otherwise, when not /, add the found directory to our path we're
				# in, meaning we're currently 1 directory deeper in the hierarchy.
				#
				elif line[5:] != "/":
					#print ("Go to path %s" % path+"-"+line[5:])
					self.path.append(currentPath.rstrip('/')+'/'+line[5:])

			# When the line doesn't start with a $, get the last item from the path.
			# This will be the directory we're currently in.
			#
			if line[:1] != "$":

				# Check if the directory is found in the filesystem-object already.
				# If not, go create an entry for it, holding the dirs and files and
				# a size (set to None)
				#
				if not currentPath in self.filesystem:
					self.filesystem[currentPath] = {'dirs': {}, 'files': {}, 'size': None}

				# When the line starts with 'dir ', extract the name of the dir and
				# add it onto the dirs-element of tot current path.
				#
				if line[:4] == "dir ":
					self.filesystem[currentPath]['dirs'][currentPath.rstrip('/')+"/"+line[4:]] = None

				# Else, we're dealing with a file. Split it by a space in order to
				# extract the fileSize and the fileName from it and add it onto the
				# files-element of tot current path.
				#
				else:
					file = line.split(" ")
					self.filesystem[currentPath]['files'][file[1]] = int(file[0])

		# At the end, when we're done reading the whole filesystem, call getPathSize(/)
		# which will recursively loop through all child directories, get their sizes as well
		# by calling getPathSize again and so on and so on
		#
		self.getPathSize("/")
	...
	def getPathSize(self, path):
		...
		# Check if we've already got a size for the given path.
		#
		if self.filesystem[path]['size'] == None:

			size = 0

			# If not, loop through all directories. Check if we've
			# already got a size for it. If not, call getPathSize
			# for the found directory. Eventually, add the size of
			# the directory to the size variable
			#
			for directory in self.filesystem[path]['dirs']:
				if self.filesystem[path]['dirs'][directory] is None:
					self.filesystem[path]['dirs'][directory] = self.getPathSize(directory)

				size += self.filesystem[path]['dirs'][directory]

			# And loop through all files, add the size of the file
			# to the size variable
			#
			for file in self.filesystem[path]['files']:
				size += self.filesystem[path]['files'][file]

			# Update the size for the path
			#
			self.filesystem[path]['size'] = size

		# Return the stored or calculated size.
		#
	...
	def part2(self):
		...
		# 
		freeDiskSpace = 70000000 - self.getPathSize("/")
		requiredSpace = 30000000 - freeDiskSpace
