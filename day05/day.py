...
class puzzle:
	...
	def handleInput(self):
		...
		for line in self.lines:
			...
			# Check if the line contains info about the initial crates
			#
			if line.strip()[0:1] == "[":
			...
			# Check if the line contains info about the movements to be executed
			#
			if line[0:4] == "move":

				move = re.search(r"move ([0-9]+) from ([0-9]+) to ([0-9]+)", line)
				self.movements.append({"move": int(move.group(1)), "from": int(move.group(2)), "to": int(move.group(3))})
	...
	def part1(self):
		...
		# Perform a (deep)copy for the self.stacks-object, so in part 2
		# of the puzzle, we can use the same object again, but now using
		# the CrateMover 9001 instead.
		#
		...
		for move in self.movements:
			...
			for i in range(1, move['move']+1):
				...
				# Get crate from stack 'from'
				#
				crate = stacks[move['from']].pop(0)

				# Add crate onto stack 'to'
				#
				stacks[move['to']].insert(0, crate)

		return self.getTopCreates(stacks)
	...
	def part2(self):
		...
		for move in self.movements:
			...
			#
			for i in range(move['move'], 0, -1):

				# Get crate from stack 'from'
				#
				crate = stacks[move['from']].pop(i-1)
	...
	#
	def getTopCreates(self, stacks):

		answer = ""
		for stack in stacks:
			answer += stacks[stack].pop(0)
