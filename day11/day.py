...
class puzzle:
	...
	def handleInput(self):
		...
		for monkey in monkeys:
			...
			self.monkeys[int(matches.groups()[0])] = {
				...
				"divisibleBy": int(matches.groups()[5]),
				"trueMonkey": int(matches.groups()[6]),
				"falseMonkey": int(matches.groups()[7]),
				"inspections": 0
			};

			# Multiple the divisions with each divisibleBy-value.
			# For example, when we must divide by 2 for monkey 0,
			# divide by 3 for monkey 1, and divide by 5 for monkey
			# 2, the result will be 30 (2 * 3 * 5)
			#
			self.divisions = int(matches.groups()[5]) * self.divisions

		return

	#
	def handleRound(self, dividedByThree=True, debug=False):

		for monkey in self.monkeys:
			if debug: 
				print("Monkey %d" % monkey)
			attributes = self.monkeys[monkey]
			...
			for item in attributes["items"]:
				...
				self.monkeys[monkey]["inspections"] += 1
				self.handleItem(monkey, item, dividedByThree, debug)

			self.monkeys[monkey]["items"] = []

	#
	def handleItem(self, monkey, worryLevel, dividedByThree=True, debug=False):
		attributes = self.monkeys[monkey]

		if attributes["operation"][2] == 'old':
			operationBy = worryLevel
		else:
			operationBy = int(attributes["operation"][2])

		if attributes["operation"][1] == "*":
			worryLevel = int(worryLevel) * operationBy
			if debug:
				print("    Worry level is multiplied by %s to %s." % (operationBy, worryLevel))
		elif attributes["operation"][1] == "+":
			worryLevel = int(worryLevel) + operationBy
			if debug:
				print("    Worry level increases by %s to %s." % (operationBy, worryLevel))			
		...
		if dividedByThree:
			...
			if debug:
				print("    Monkey gets bored with item. Worry level is divided by 3 to %d." % worryLevel)	
		
		# --- Part Two --- 
		# Worry levels are no longer divided by three after each item is inspected;
		# By modulo the worryLevel by all divisions multiplied together, we'll
		# get a result, which can be used in the next round, but also gives the
		# same result.
		# 
		# For example, when all divisions multiplied together is 30, (because we've
		# got a monkey with divide by 5, a monkey with divide by 2 and a monkey with
		# divide by 3).
		#
		# WorryLevel 14, Worrylevel 44 (14 + 30), WorryLevel 74 (14 + 30 + 30) will
		# all given the same results in ht next rounds. So by using modulo, we can
		# keep the worry levels manageable.
		# 
		else:
			if(operationBy > 0):
				worryLevel = worryLevel % self.divisions

		if worryLevel % attributes["divisibleBy"] == 0:
			if debug:
				print("    Current worry level is divisible by %d." % attributes["divisibleBy"])
			if debug:
				print("    Item with worry level %d is thrown to monkey %d." % (worryLevel, attributes["trueMonkey"]))
			self.monkeys[attributes["trueMonkey"]]["items"].append(worryLevel)
		else:
			if debug:
				print("    Current worry level is not divisible by %d." % attributes["divisibleBy"])
			if debug:
				print("    Item with worry level %d is thrown to monkey %d." % (worryLevel, attributes["falseMonkey"]))
			self.monkeys[attributes["falseMonkey"]]["items"].append(worryLevel)
	...
	def part1(self):
		...
		for round in range(1,21):
			self.handleRound(True)

		inspections = []
		for monkey in self.monkeys:
			inspections.append(self.monkeys[monkey]["inspections"])
	...
	def part2(self):
		...
		# Call handleInput again to make sure all monkeys have the right
		# starting items again.
		#
		self.handleInput()

		for round in range(1,10001):
			self.handleRound(False)
